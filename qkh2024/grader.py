...
class scorer:
    ...
    def grader_connection_check(
        ...
        for keys, items in gate_conn_set_circuit.items():
            check_connections.append(
                int(gate_conn_set_circuit[keys].issubset(gate_conn_set_backend[keys]))
            )
    ...
    def grader_circuit_accuracy(
        ...
        circuit_target = transpile(
            self._remove_idle_qwires(circuit_target), backend=simulator
        )
        circuit_transpiled = transpile(
            self._remove_idle_qwires(circuit_transpiled), backend=simulator
        )
    ...
    def _connections_circuit(self, circ: QuantumCircuit) -> dict[str, list]:
        """
